...
def get_city_year (p0, perc, delta, target_p):
    current_number = p0
    my_counter=0
    second_year = current_number + current_number*perc/100+delta
    if second_year<current_number:
        print(-1)
    else:
        while current_number<target_p:
            current_number = current_number +current_number*perc/100+delta
            my_counter += 1
       
        return my_counter

printesanai = get_city_year(1000, -3, 50, 2000)
